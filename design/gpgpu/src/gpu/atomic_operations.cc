...
AtomicOpRequest::atomicMemoryAccess(PacketPtr pkt, memory::SimpleMemory *phys_mem)
{
    ...
    //                   pkt->req->getFlags(), 0);
    RequestPtr atomic_req = std::make_shared<Request>(pkt->getAddr(), data_size_bytes,
                       pkt->req->getFlags(), 0);
    ...
    for (int i = 0; !atomics_done; i++) {
        ...
        uint8_t read_data[8];
        // TODO schi Packet atomic_read_pkt(&atomic_req, MemCmd::ReadReq, data_size_bytes);
        PacketPtr atomic_read_pkt = new Packet(atomic_req, MemCmd::ReadReq, data_size_bytes);

        uint8_t write_data[8];
        // Packet atomic_write_pkt(&atomic_req, MemCmd::WriteReq, data_size_bytes);
        PacketPtr atomic_write_pkt = new Packet(atomic_req, MemCmd::WriteReq, data_size_bytes);

        assert(atomic_ops[i]->dataSizeBytes() == data_size_bytes);

        // Set up the read packet to get the correct data
        // TODO schi atomic_read_pkt.setAddr(pkt->getAddr() + atomic_ops[i]->lineOffset);
        // TODO schi atomic_read_pkt.dataStatic(read_data);
        ...
        // TODO schi phys_mem->access(&atomic_read_pkt);
        ...
        DPRINTF(AtomicOperations, "Performing operation for addr: %x\n",
                ...
                // TODO schi atomic_read_pkt.getAddr());
